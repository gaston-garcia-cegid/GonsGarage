...
- **Backend:** monolito modular en **Go**, **Clean Architecture**: transporte (`internal/handler`, `cmd/`, `internal/middleware`), reglas (`internal/service`), entidades (`internal/domain`), persistencia (`internal/repository` + **PostgreSQL**), utilidades (`pkg/`). *Fase 1: handler y middleware ya viven en esos paquetes; `domain` / `service` / `repository` siguen como fachadas (aliases) sobre `internal/core` y `internal/adapters` — ver `docs/template-adoption-plan.md`.*
